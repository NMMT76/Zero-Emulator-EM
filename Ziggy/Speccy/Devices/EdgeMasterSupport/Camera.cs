...
namespace Speccy.Devices.EdgeMasterSupport
{
	public class Capture : ISampleGrabberCB, IDisposable
	{
		...
		private Capture(string devicename, int iWidth, int iHeight, short iBPP, Control hControl)
		{
			DsDevice[] capDevices;
			...
			{
				if (capDevices[i] != null && capDevices[i].Name == devicename)
				{
					iDeviceNum = i;
					break;
				}
			...
			if (iDeviceNum==-1) //Device not found
			{
		...
		public static Capture New(string devicename, int iWidth, int iHeight, short iBPP, Control hControl)
		{
			Capture device = null;
			try
			{
				device = new Capture(devicename,iWidth,iHeight,iBPP,hControl);
			}
			catch (Exception ex)
			{
				return null;
